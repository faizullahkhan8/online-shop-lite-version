...
import { readGuestOrders } from "../utils/guestOrders";
import {
    Loader2, Receipt, ChevronRight, Search, CheckCircle2,
    Package, MapPin, CreditCard, Calendar, User, Phone,
    Truck, Star, Leaf,
} from "lucide-react";
...
const getStatusStyle = (status) => {
    switch (status?.toLowerCase()) {
        case "delivered":  return "bg-[#eef6ea] text-[#2e6b2e] border border-[#c3e0b8]";
        case "shipped":    return "bg-[#eff6ff] text-[#1e4d8c] border border-[#bfdbfe]";
        case "pending":    return "bg-[#fefce8] text-[#854d0e] border border-[#fde68a]";
        case "cancelled":  return "bg-[#fff1f2] text-[#9f1239] border border-[#fecdd3]";
...
const TrackOrderPage = () => {
    ...
    const { data, isLoading: loading, error } = useOrderById(selectedOrderId, {
        enabled: !!selectedOrderId,
    });
                ...
                >
                    <form onSubmit={handleSubmit} className="flex gap-3">
                        ...
                        <motion.button
                            type="submit"
                            disabled={loading}
                            whileHover={{ scale: 1.03, backgroundColor: "#2e4a2e" }}
                            whileTap={{ scale: 0.97 }}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-8 space-y-5">
                        ...
                        {loading && (
                            <div className="bg-white border border-[#e8f0e4] rounded-3xl p-20 flex flex-col items-center justify-center gap-4">
                                ...
                                <p className="text-[10px] uppercase tracking-[0.35em] text-stone-400">Fetching your ritual…</p>
                            </div>
                        )}
                        ...
                        {!loading && !order && !error && (
                            ...
                            >
                                <div className="w-16 h-16 rounded-full bg-[#eef6ea] flex items-center justify-center mb-5">
                                    <Package size={22} strokeWidth={1} className="text-[#b5d4a6]" />
                                </div>
                        ...
                        {!loading && order && (
                            <AnimatePresence>
                                <div className="space-y-5">
                                    ...
                                    {/* ── Order Summary Card ── */}
                                    <motion.div
                                        custom={0} variants={cardFade} initial="hidden" animate="visible"
                                        className="bg-white border border-[#e8f0e4] rounded-3xl p-6 md:p-8"
                                    >
                                        <div className="flex items-start justify-between mb-6">
                                            <div>
                                                <p className="text-[10px] uppercase tracking-[0.3em] text-[#7aaf68] mb-1">Order</p>
                                            ...
                                            <span className={`px-4 py-1.5 rounded-full text-[10px] font-medium uppercase tracking-widest ${getStatusStyle(order.status)}`}>
                                                {order.status || "Processing"}
                                            </span>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 pt-6 border-t border-[#f0f5ee]">
                                            <div className="flex items-start gap-3">
                                                <div className="w-8 h-8 rounded-xl bg-[#eef6ea] flex items-center justify-center shrink-0">
                                                    <CreditCard size={14} className="text-[#7aaf68]" />
                                                </div>
                                                <div>
                                                    <p className="text-[10px] text-stone-400 uppercase tracking-widest mb-1">Grand Total</p>
                                            ...
                                            <div className="flex items-start gap-3">
                                                <div className="w-8 h-8 rounded-xl bg-[#eef6ea] flex items-center justify-center shrink-0">
                                                    <Truck size={14} className="text-[#7aaf68]" />
                                                </div>
                                                <div>
                                                    <p className="text-[10px] text-stone-400 uppercase tracking-widest mb-1">Shipping Fee</p>
                                            ...
                                            <div className="flex items-start gap-3">
                                                <div className="w-8 h-8 rounded-xl bg-[#eef6ea] flex items-center justify-center shrink-0">
                                                    <Calendar size={14} className="text-[#7aaf68]" />
                                                </div>
                                                <div>
                                                    <p className="text-[10px] text-stone-400 uppercase tracking-widest mb-1">Payment</p>
                                    ...
                                    {/* ── Delivery Information ── */}
                                    <motion.div
                                        custom={0.08} variants={cardFade} initial="hidden" animate="visible"
                                        className="bg-white border border-[#e8f0e4] rounded-3xl p-6 md:p-8"
                                    >
                                        <div className="flex items-center gap-3 mb-6">
                                            <div className="w-8 h-8 rounded-xl bg-[#eef6ea] flex items-center justify-center">
                                                <MapPin size={14} className="text-[#7aaf68]" />
                                            </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                            <div className="flex items-start gap-3">
                                                <User size={15} className="text-stone-300 mt-0.5" />
                                                <div>
                                            ...
                                            <div className="flex items-start gap-3">
                                                <Phone size={15} className="text-stone-300 mt-0.5" />
                                                <div>
                                            ...
                                            <div className="md:col-span-2 flex items-start gap-3">
                                                <MapPin size={15} className="text-stone-300 mt-0.5" />
                                                <div>
                                                    <p className="text-[10px] text-stone-400 uppercase tracking-widest mb-1">Delivery Address</p>
                                                    <p className="text-sm text-[#1a2e1a] font-medium">
                                                        {order.recipient?.street}
                                                        {order.recipient?.addressLine2 && `, ${order.recipient.addressLine2}`}
                                                    </p>
                                                    <p className="text-xs text-stone-400 mt-0.5">
                                                        {[order.recipient?.city, order.recipient?.state, order.recipient?.postalCode, order.recipient?.country]
                                                            .filter(Boolean).join(", ")}
                                                    </p>
                                                </div>
                                            </div>
                                    ...
                                    {/* ── Order Items ── */}
                                    <motion.div
                                        custom={0.16} variants={cardFade} initial="hidden" animate="visible"
                                        className="bg-white border border-[#e8f0e4] rounded-3xl p-6 md:p-8"
                                    >
                                        <div className="flex items-center gap-3 mb-6">
                                            <div className="w-8 h-8 rounded-xl bg-[#eef6ea] flex items-center justify-center">
                                                <Package size={14} className="text-[#7aaf68]" />
                                            </div>
                                            <h3 className="text-xs font-medium uppercase tracking-[0.25em] text-[#1a2e1a]">
                                                Order Items
                                            </h3>
                                                    ...
                                                    >
                                                        <div className="flex items-start justify-between mb-3">
                                                            <div>
                                                                {productName ? (
                                                                    <>
                                                                        <p className="text-sm font-medium text-[#1a2e1a]">{productName}</p>
                                                                        <p className="text-[10px] text-stone-400 mt-0.5">ID: {productId}</p>
                                                                    </>
                                                                ) : (
                                                                    <p className="text-sm font-medium text-[#1a2e1a]">Product ID: {productId}</p>
                                                                )}
                                                                <p className="text-xs text-stone-400 mt-1">Qty: {item.quantity}</p>
                                                            </div>
                                                            <span className={`px-3 py-1 rounded-full text-[10px] uppercase tracking-widest font-medium ${
                                                        ...
                                                        {order.status === "delivered" &&
                                                            ...
                                                            <button
                                                                onClick={() => {
                                                                    setSelectedProduct({ _id: productId, name: productName || "Product" });
                                                                    setIsReviewModalOpen(true);
                                                                }}
                                                                className="mb-3 text-[10px] font-medium uppercase tracking-widest text-[#7aaf68] hover:text-[#5a9050] transition-colors flex items-center gap-2 border border-[#c3e0b8] bg-[#eef6ea] px-3 py-1.5 rounded-xl w-fit"
                                                            >
                                                                <Star size={11} className="fill-[#7aaf68]" />
                                                                Give Review
                                                            </button>
                                                        )}
                                                        ...
                                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs pt-3 border-t border-[#e8f0e4]">
                                                            <div>
                                                                ...
                                                                <p className="font-medium text-[#1a2e1a]">RS {Number(item.price || 0).toLocaleString()}</p>
                                                            </div>
                                                            {item.discount > 0 && (
                                                                <div>
                                                                    <p className="text-stone-400 mb-0.5">Discount</p>
                                                                    <p className="font-medium text-[#2e6b2e]">-RS {item.discount}</p>
                                                                </div>
                                                            )}
                                                            {item.promotion?.title && (
                                                                <div className="md:col-span-2">
                                                                    <p className="text-stone-400 mb-0.5">Promotion</p>
                                                                    <p className="font-medium text-[#1e4d8c]">{item.promotion.title}</p>
                                                                </div>
                                                            )}
                                                            <div>
                                                                <p className="text-stone-400 mb-0.5">Total</p>
                                                                <p className="font-semibold text-[#1a2e1a] text-sm">RS {Number(item.totalAmount || 0).toLocaleString()}</p>
                                                            </div>
                                                        </div>
                                        ...
                                        <div className="mt-6 pt-5 border-t border-[#eef3eb] space-y-2">
                                            {order.taxAmount > 0 && (
                                                <div className="flex justify-between text-xs text-stone-500">
                                                    ...
                                                    <span>RS {Number(order.taxAmount || 0).toLocaleString()}</span>
                                                </div>
                                            )}
                                            <div className="flex justify-between text-xs text-stone-500">
                                            ...
                                            <div className="flex justify-between text-sm font-medium text-[#1a2e1a] pt-3 border-t border-[#e8f0e4]">
                                                ...
                                                <span className="text-lg font-light">RS {Number(order.grandTotal || 0).toLocaleString()}</span>
                                            </div>
                                        </div>
                                    </motion.div>
                                    ...
                                    {order.status === "cancelled" && order.cancellationReason && (
                                        <motion.div
                                            custom={0.24} variants={cardFade} initial="hidden" animate="visible"
                    ...
                    <div className="lg:col-span-4">
                        ...
                        >
                            <div className="flex items-center gap-3 mb-6">
                                <div className="w-8 h-8 rounded-xl bg-[#eef6ea] flex items-center justify-center">
                                    <Receipt size={14} className="text-[#7aaf68]" />
                                </div>
                            ...
                            {savedOrders.length === 0 ? (
                                <div className="text-center py-10">
                                    <div className="w-12 h-12 rounded-full bg-[#f8faf7] flex items-center justify-center mx-auto mb-3">
                                        <Receipt size={18} strokeWidth={1} className="text-stone-300" />
                                    </div>
                                    <p className="text-xs text-stone-400 tracking-wide">No recent orders found</p>
                                </div>
                            ) : (
                                <div className="space-y-2">
                                    {savedOrders.map((id) => (
                                        <motion.button
                                            key={id}
                                            onClick={() => handleSavedOrderClick(id)}
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                <CheckCircle2 size={14} className={selectedOrderId === id ? "text-[#7aaf68]" : "text-stone-300"} />
