...
const PromotionBar = () => {
    ...
    const remainingTime = activePromotion?.endTime
        ? formatRemainingTimeTabular(new Date(activePromotion.endTime), new Date(now))
        : "";
    ...
    if (!activePromotion || (new Date(activePromotion.endTime) <= new Date())) return;
    ...
    return (
        <div
            onClick={() => { ScrollButton(activePromotion._id)}}
